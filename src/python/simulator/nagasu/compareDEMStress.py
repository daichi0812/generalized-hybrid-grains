...
class CompareDEMStressGrid:
    def __init__(self):
        self.grid_start = np.zeros(2, dtype=ctypes.c_float)
        self.grid_start_from_tool = np.zeros(2, dtype=ctypes.c_float)
        self.h = 0.01
        self.resolution = np.zeros(2, dtype=ctypes.c_int)
        self.resolution_from_tool = np.zeros(2, dtype=ctypes.c_int)
        self.sigma = np.zeros((1, 2, 2), dtype=ctypes.c_float)
        self.sigma_from_tool = np.zeros((1, 2, 2), dtype=ctypes.c_float)

    def setData(self, homogenization_data_from_tool, force_data, scene_data):
        #load homogenize stress from tool
        self.grid_start_from_tool = homogenization_data_from_tool.homogenization[0].grid_p
        self.h = homogenization_data_from_tool.homogenization[0].h
        self.resolution_from_tool = homogenization_data_from_tool.homogenization[0].resolution
        N = np.prod(self.resolution_from_tool)
        self.sigma_from_tool = np.zeros((N, 2, 2), dtype=ctypes.c_float)
        for j in range(self.resolution_from_tool[1]):
            for i in range(self.resolution_from_tool[0]):
                flat_idx = j * self.resolution_from_tool[0] + i
                self.sigma_from_tool[flat_idx] = homogenization_data_from_tool.homogenization[flat_idx].sigma

        #compute homogenize stress
        homogenize_stress = HomogenizationGrid()
        homogenization_data = HomogenizeData()
        offset_ratio = np.array([0.0, 0.0])
        homogenize_stress.setData(offset_ratio, self.h, force_data, homogenization_data)
        ...
        for j in range(self.resolution[1]):
            for i in range(self.resolution[0]):
                ...
                self.sigma[flat_idx] = homogenization_data.homogenization[flat_idx].sigma
    ...
    def computeVBOsForDraw(self, VBO):
        ...
        for j in range(self.resolution_from_tool[1]):
            for i in range(self.resolution_from_tool[0]):
                flat_idx = j * self.resolution_from_tool[0] + i
                if (np.isnan(self.sigma_from_tool[flat_idx])).any() == True:
                    vc = self.grid_start_from_tool + self.h * np.array([i + 0.5, j + 0.5])
                    v0 = vc + self.h * np.array([0.5, 0.5])
                    v1 = vc + self.h * np.array([-0.5, -0.5])
                    color = [255.0 / 255.0, 255.0 / 255.0, 255.0 / 255.0]
                    VBO.registerLineToDataArray(v0, v1, color, color)
                    continue
                l, Q = np.linalg.eig(self.sigma_from_tool[flat_idx])
                vc = self.grid_start_from_tool + self.h * np.array([i + 0.5, j + 0.5])
        ...
        for j in range(self.resolution[1]):
            for i in range(self.resolution[0]):
                ...
                if (np.isnan(self.sigma[flat_idx])).any() == True:
                    ...
                    v0 = vc + self.h * np.array([0.5, 0.5])
                    v1 = vc + self.h * np.array([-0.5, -0.5])
                    color = [0.0 / 255.0, 255.0 / 255.0, 255.0 / 255.0]
                    VBO.registerLineToDataArray(v0, v1, color, color)
                    continue
                ...
                if l[0] < 0.0:
                    color = [0.0 / 255.0, 40.0 / 255.0, 245.0 / 255.0]
                VBO.registerLineToDataArray(v0, v1 , color, color)
...
class QTGLWidget2(QGLWidget):
    def __init__(self, parent):
        ...
        self.homogenization_grid = CompareDEMStressGrid()
    ...
    def setData(self, homogenization_data, force_data, scene_data):
        self.homogenization_data = copy.deepcopy(homogenization_data)
        self.force_data = copy.deepcopy(force_data)
        self.data = copy.deepcopy(scene_data)
        self.homogenization_grid.setData(self.homogenization_data, self.force_data, self.data)
        self.autoAdjustView()
        self.buildVBO()
        self.updateGL()

    def updateHomogenization(self, homogenization_data, force_data, scene_data):
        self.homogenization_data = copy.deepcopy(homogenization_data)
        self.force_data = copy.deepcopy(force_data)
        self.data = copy.deepcopy(scene_data)
        self.homogenization_grid.setData(self.homogenization_data, self.force_data, self.data)
...
class Ui_Widget(QtWidgets.QWidget):
    ...
    def update_homogenization(self):
        dt_idx = int(self.timestep_idx_slider.value())
        self.homogenization_stress_data.load_from_idx(self.h_fn1, dt_idx)
        self.force_data.load_from_idx(self.f_fn, dt_idx + 1)
        self.scene_data.load_from_idx(self.t_fn, self.f_fn, dt_idx)
        self.gl.setData(self.homogenization_stress_data.all_step_homogenization[0], self.force_data.all_step_ForceData[0], self.scene_data.all_step_elements[0])
    ...
    def load(self):
        ...
        (self.h_fn1, selectedFilter) = QtWidgets.QFileDialog.getOpenFileName(self, "Load  homogenization stress", os.path.expanduser('~'), "HDF5 (*.h5)")
