...
class Ui_Widget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.scroll_area_vbox = QtWidgets.QVBoxLayout()

        for t in self.scene_data.templates:
            self.updateTemplateLayout(t)

        inner = QtWidgets.QWidget()
        inner.resize(500, inner.height())
        inner.setLayout(self.scroll_area_vbox)
        ...
        self.min_x_slider.setMaximum(1000)
        self.min_x_slider.setValue(500)
        ...
        self.max_x_slider.setMaximum(1000)
        self.max_x_slider.setValue(500)
        ...
        self.min_y_slider.setMaximum(1000)
        self.min_y_slider.setValue(500)
        ...
        self.total_mass_slider.setMaximum(10000)
        self.total_mass_slider.setValue(5000)
        ...
        add_btn = QtWidgets.QPushButton('Add Template', self)
        add_btn.clicked.connect(self.addTemplate)
    ...
    def updateTemplateLayout(self, t):
        ...
        size_mean_slider.setMaximum(1000)
        size_mean_slider.setValue(500)
        ...
        self.scroll_area_vbox.addWidget(groupbox)
    ...
    def updateTemplateSetting(self):
        for i in range(len(self.template_names)):
            ...
            self.size_mean_labels[i].setText('size_mean: %.5f' % size_mean)
            ...
            self.scene_data.templates[self.template_names[i]].size_std = size_std
        ...
        self.min_x_label.setText('min x: %.4f' % self.getExtentCoord(self.min_x_slider.value()))
        self.max_x_label.setText('max x: %.4f' % self.getExtentCoord(self.max_x_slider.value()))
        self.min_y_label.setText('min y: %.4f' % self.getExtentCoord(self.min_y_slider.value()))
    ...
    def generate(self):
        ...
        print(self.getMass(self.total_mass_slider.value()))
        ...
        while total_mass < target_mass:
            ...
            print("size_ratio:"+ str(size_ratio))
            print("mass:" + str(mass))
    ...
    def addTemplate(self):
        add_scene_data = SceneData()
        # template
        (path_to_template, selectedFilter) = QtWidgets.QFileDialog.getOpenFileName(self, "Load templates",
                                                                                   os.path.expanduser('~'),
                                                                                   "HDF5 (*.h5)")
        # elements
        (path_to_elements, selectedFilter) = QtWidgets.QFileDialog.getOpenFileName(self, "Load elements",
                                                                                   os.path.expanduser('~'),
                                                                                   "HDF5 (*.h5)")

        add_scene_data.load(path_to_template, path_to_elements)
        self.scene_data.templates.update(add_scene_data.templates)
        for t in add_scene_data.templates:
            self.updateTemplateLayout(t)
        self.updateTemplateSetting()
