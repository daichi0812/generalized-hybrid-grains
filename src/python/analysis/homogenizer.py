...
class HomogenizationGrid:
    ...
    def setData(self, grid_start_offset_ratio, h, force_data, homogenization_data):
        if len(force_data.forces) == 0:
            return
        ...
        self.homogenized_stress = np.zeros((N,2,2), dtype=ctypes.c_float)
        
        #homogenization
        #compute homogenize stress (hybrid_grains algorithm 11)
        cell_volume = self.h**2
        for f in force_data.forces:
            cell_idx = np.floor((f.position - self.grid_start) / h).astype(int)
            flat_idx = cell_idx[1] * self.resolution[0] + cell_idx[0]
        #Christoffesen formula
            self.sigma[flat_idx] += 0.5 * (np.outer(f.arm, f.force) + np.outer(f.force, f.arm)) / cell_volume  #REVIEW:重みづけ、接触点周囲に影響
        ...
        for j in range(self.resolution[1]):
            for i in range(self.resolution[0]):
                ...
                k.grid_p = self.grid_start + h * np.array([i, j])
...
class QTGLWidget2(QGLWidget):
    ...
    def buildVBO(self):
        ...
        for e in self.data.elements:
            ...
            if num_vertices < 3:
                self.computeVBOsForDrawingCircle(self.grains_VBO, e.center_of_mass, e.size_ratio * 0.5 * self.data.templates[e.template_name].size_mean, e.rotation_angle, e.static)
            else:
                self.computeVBOsForDrawingPolygon(self.grains_VBO, e.center_of_mass, e.size_ratio, self.data.templates[e.template_name].vertex_list, e.rotation_angle, e.static)
    ...
    def autoAdjustView(self):
        ...
        for e in self.data.elements:
            ...
            num_vertices_shape = vertex_list_e.shape[1]
            if num_vertices_shape < 3:
                _v_min, _v_max = self.computeCircleBB(e.center_of_mass, e.size_ratio * 0.5 * self.data.templates[e.template_name].size_mean)
                v_min = np.minimum(v_min, _v_min)
                v_max = np.maximum(v_max, _v_max)
            else:
                _v_min, _v_max = self.computePolygonBB(e.center_of_mass, e.size_ratio, self.data.templates[e.template_name].vertex_list, e.rotation_angle)
                v_min = np.minimum(v_min, _v_min)
                v_max = np.maximum(v_max, _v_max)
    ...
    def setData(self,  force_data, homogenization_data):
        self.force_data = copy.deepcopy(force_data)
        self.homogenization_grid.setData(np.array([0.0, 0.0]), 0.04, self.force_data, homogenization_data)
        self.autoAdjustView()
        self.buildVBO()
        self.updateGL()
...
class Ui_Widget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.homogenization_data_test = HomogenizeData()
    ...
    def load(self):
        ...
        self.force_data.load(element_fn2)
        self.gl.setData(self.force_data, self.homogenization_data)
