...
class GrainRender:
    ...
    def drawElements(self, context, line_width):
        for e in self.data.elements:
            ...
            if num_vertices < 3:
                self.drawCircle(context, line_width, e.center_of_mass, e.size_ratio * 0.5 * self.data.templates[e.template_name].size_mean, e.rotation_angle, e.static)
            else:
                self.drawPolygon(context, line_width, e.center_of_mass, e.size_ratio, self.data.templates[e.template_name].vertex_list, e.rotation_angle, e.static)
    ...
    def computeAverageSize(self):
        ...
        for e in self.data.elements:
            ...
            count += 1
            if num_vertices < 3:
                _bb_min, _bb_max = self.computeBBForCircle(e.center_of_mass, e.size_ratio * 0.5 * self.data.templates[e.template_name].size_mean)
                bb_sz += _bb_max - _bb_min
            else:
                _bb_min, _bb_max = self.computeBBForPolygon(e.center_of_mass, e.size_ratio, self.data.templates[e.template_name].vertex_list, e.rotation_angle)
                bb_sz += _bb_max - _bb_min
    ...
    def setData(self, input_node_data, output_node_data, render_cycle):
        max_count = 10000000
        range_max = input_node_data.end + 1
        if range_max <= 0:
            range_max = max_count

        output_data_number = 0
        #print("range_max:", range_max)

        cycle = int(render_cycle)
        for i in range(input_node_data.start, range_max):
            if (i % cycle) == 0:
                #print(i)
                elements_fn = input_node_data.elements_template % i
                if not os.path.exists(elements_fn):
                    return

                basename_without_ext = os.path.splitext(os.path.basename(elements_fn))[0]
                #output_basename = basename_without_ext + '.' + output_node_data.type
                output_basename = str(output_data_number) + ".png"
                output_fn = os.path.join(output_node_data.folder, output_basename)

                print("##### processing ", i, " #####")
                print("  templates: ", input_node_data.templates_fn)
                print("  elements: ", elements_fn)
                print("  output: ", output_fn)

                self.data.load(input_node_data.templates_fn, elements_fn)
                self.render(input_node_data.bb, output_node_data.type, output_fn, output_node_data.size)
                output_data_number += 1
...
class GrainRenderManager:
    ...
    def render(self, render_cycle):
        self.renderer.setData(self.inputNodeData, self.outputNodeData, render_cycle)
